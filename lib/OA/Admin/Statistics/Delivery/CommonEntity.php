...
//=========================Modified By DAC002 && DAC021==================================================//


	     if (is_numeric($bannerId)) {
				$query = mysql_query("select * from rv_banners where bannerid=".$bannerId." and masterbanner = -2") or die(mysql_error());
				if(mysql_num_rows($query))
					{	
						$banner['masterbanner'] = -2;
					}
				else
					{
						$banner['masterbanner'] = 0;
					}
				    }		
//=========================Modified By DAC002 && DAC021==================================================//
                $banner['prefix'] = 'b';
    ...
    function getZones($aParams, $level, $expand)
    {
        ...
        $aEntitiesData = array();
//=========================Modified By DAC002 ==================================================//
	$query_zones = mysql_query("select * from rv_zones where masterzone= -2") or die(mysql_error());
	while($fetch_zones=mysql_fetch_assoc($query_zones)) {
		if($aZones[$fetch_zones['zoneid']]['zone_id']==$fetch_zones['zoneid']) {
		$aZones1[$fetch_zones['zoneid']]=$aZones[$fetch_zones['zoneid']];
		$aZones1[$fetch_zones['zoneid']]['zone_id']=$fetch_zones['zoneid'];
		}
	$query_mobile_zones = mysql_query("select * from rv_zones where masterzone=".$fetch_zones['zoneid']) or die(mysql_error());
		while($fetch_mobile_zones=mysql_fetch_assoc($query_mobile_zones)) {
			if($aZones[$fetch_mobile_zones['zoneid']]['zone_id']==$fetch_mobile_zones['zoneid']) {
			$aZones1[$fetch_mobile_zones['zoneid']]=$aZones[$fetch_mobile_zones['zoneid']];
			//$aZones1[$fetch_zones['zoneid']]['zone_id']=$fetch_zones['zoneid'];
			$aZones1[$fetch_mobile_zones['zoneid']]['zone_id']=$fetch_mobile_zones['zoneid'];
			}
		}
	}
		$aZones=NULL;
...
//=========================Modified By DAC002 && DAC021==================================================//
				$query = mysql_query("select * from rv_zones where zoneid=".$zoneId." and masterzone = -2") or die(mysql_error());
				if(mysql_num_rows($query)) {	
						$zone['masterzone'] = -2;
					}
				else {
