...
class OA_Creative_File extends OA_Creative
{
    ...
    function &factoryUploadedFile($variableName) {
        ...
        if (@is_readable($filePath)) {
			 ...
			 $tempfilename	=	tempnam(MAX_PATH . '/var','oa_creative_');
           ...
           //$tmpName = tempnam(MAX_PATH . '/var', 'oa_creative_');
            if (!file_exists($tempfilename) || $tempfilename === false) { 
                return new PEAR_Error('Cannot create a temporary file: '.$filePath);
            }
            // Move uploaded file to a temporary file
            if (!@move_uploaded_file($filePath, $tempfilename)) {
                // Cleanup
                @unlink($tempfilename);
                return new PEAR_Error('Could not move the uploaded file to: '.$tempfilename);
            }
