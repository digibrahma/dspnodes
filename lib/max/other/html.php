...
require_once 'mobilelimitations.php';
...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
	    ...
	    $_zone_query = mysql_query("select * from oxm_mobilezones where masterzoneid=".$GLOBALS['zoneId'])or die("Error-->1".mysql_error());

	    if(mysql_num_rows($_zone_query)>0 ){

			$_query = mysql_query("select * from ".$table_prefix."banners where masterbanner = '-2' and campaignid = ".$otherEntityId."")or die("Error-->in mobile".mysql_error());

			$aStringParams["bannerCount"] = mysql_num_rows($_query);
	
	    } else {

   	       		$_zone = mysql_query("select masterzone from ".$table_prefix."zones where zoneid=".$GLOBALS['zoneId'])or die("Error-->in else".mysql_error());
    	   	    $_zone_det = mysql_fetch_array($_zone);

		  if($_zone_det['masterzone'] == '-1' ){

			$_zoneQuery = mysql_query("select * from ".$table_prefix."zones where zoneid = ".$GLOBALS['zoneId']."");

			$_rowzone = mysql_fetch_assoc($_zoneQuery);

			$_query = mysql_query("select * from ".$table_prefix."banners where masterbanner = '-1' and campaignid = ".$otherEntityId." and width =".$_rowzone['width']." and height=".$_rowzone['height']." ");

			$aStringParams["bannerCount"] = mysql_num_rows($_query);
			
		}else if($_zone_det['masterzone'] == '-3' ){
		
			$_query = mysql_query("select * from ".$table_prefix."banners where masterbanner = '-3' and campaignid = ".$otherEntityId."  ")or die();
...
//////////////////////// worked linking for html banner //////////////////////////////////
...
$_zoneQuery = mysql_query("select * from ".$table_prefix."zones where zoneid = ".$GLOBALS['zoneId']."");
			...
			$_query = mysql_query("select * from ".$table_prefix."banners where masterbanner = '-4' and campaignid = ".$otherEntityId." and width =".$_rowzone['width']." and height=".$_rowzone['height']." ");
...
{
 /////video////
$redirect=mysql_fetch_array(mysql_query("select delivery,width,height from rv_zones where zoneid=".$zoneId." "));
...
function MAX_displayLinkedPlacementsAds($aParams, $publisherId, $zoneId, $hideInactive, $showMatchingAds, $pageName, &$tabIndex, $directLinkedAds=false)
  ...
  //video banner  redirect  

                    
if($redirect['delivery']=='9' || $redirect['delivery']=='6')
{
	
	$adLink = (OA_Permission::isAccount(OA_ACCOUNT_ADMIN) || OA_Permission::isAccount(OA_ACCOUNT_MANAGER)) ? "<a href='videobanner-edit.php?clientid={$aPlacement['advertiser_id']}&campaignid=$placementId&bannerid=$adId'>$adName</a>" : $adName;
}
else
{
...
function MAX_displayAcls($acls, $aParams) 
{
    ...
    foreach ($deliveryLimitations as $pluginName => $plugin) {
        ...
        if($pluginName != 'deliveryLimitations:mobileCarrierLimitation:mobileCarrierLimitation'){
    ...
    if (!empty($acls)) {
        ...
        echo "<img src='" . OX::assetPath() . "/images/icon-recycle.gif' border='0' align='absmiddle'>&nbsp;
                <a href='{$url}'>{$GLOBALS['strvideoRemoveAllLimitations']}</a>&nbsp;&nbsp;&nbsp;&nbsp;
        ";
    }
...
function displayAction($acls, $aParams) 
{
    ...
    echo "<label><img src='" . OX::assetPath() . "/images/icon-acl-add.gif' align='absmiddle'>&nbsp; Add Mobile Delivery Limitations : &nbsp;";
    echo "<select name='type' accesskey='{$GLOBALS['keyAddNew']}' tabindex='".($tabindex++)."'>";

    $deliveryLimitations = OX_Component::getComponents('deliveryLimitations', null, false);


	if($GLOBALS['conf']['pluginGroupComponents']['mobileCarrierLimitation'] == 1 && $GLOBALS['conf']['pluginGroupComponents']['MobileCarrierSelection'] == 1)	{

            echo "<option value='deliveryLimitations:mobileCarrierLimitation:mobileCarrierLimitation'>Network - Mobile Carrier</option>";

	}



            echo "<option value='handset'>Handset - Manufacture</option>";
            echo "<option value='os'>Handset - Operating System</option>";
            echo "<option value='model'>Handset - Model Name</option>";
            echo "<option value='teleco'>Handset - Teleco Operator</option>";
            echo "<option value='country'>Client - Country</option>";
            echo "<option value='profile'>Client - Custom - Profile </option>";

                   
            echo "<option value='smart'>Mobile - Smartphone Targeting</option>"; /*SMARTPHONE*/     
    ...
    echo "<table border='0' width='100%' cellpadding='0' cellspacing='0'>";
    echo "<tr><td height='25' colspan='4' bgcolor='#FFFFFF'><b>Mobile Delivery Limitations</b></td></tr>";
    echo "<tr><td height='1' colspan='4' bgcolor='#888888'><img src='" . OX::assetPath() . "/images/break.gif' height='1' width='100%'></td></tr>";
...
//VideoBanner edit/new sections
	if($pageName == 'videobanner-properties.php')	
	{	
	   if ($pageName == 'videobanner-properties.php' && empty($bannerId)) {
                $tabValue = 'videobanner-edit_new';
                $pageType = 'edit-new';
	    }
	    else {
	    	$pageType = 'edit';
...
//VideoBanner edit/new sections
	...
	if($aBanner['type'] == "mraid" || empty($aBanner['type'])){
	    $aBanner['type'] = "web";
	}
...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix']; 
 ...
 $res1=mysql_query("select * from ".$table_prefix."banners where masterbanner='-1' and bannerid=".$bannerId)or die("Error--->1".mysql_error());
...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix']; 
 ...
 $res1=mysql_query("select * from ".$table_prefix."zones where masterzone='-1' and zoneid=".$zoneid)or die("Error--->1".mysql_error());
   if(mysql_num_rows($res1) >0){   
