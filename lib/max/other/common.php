...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
	...
	$_zone_query = mysql_query("select * from oxm_mobilezones where masterzoneid=".$zoneId)or die("Error-->1".mysql_error());
	    ...
	    if(mysql_num_rows($_zone_query)>0){
			...
			foreach($aAds as $key => $_aAds) {


			$_query = mysql_query("select * from ".$table_prefix."banners where masterbanner = '-2' and bannerid = ".$key."")or die('in condition');
...
else if($_zone_det['masterzone'] == '-4' ){//fot mobile html banner
