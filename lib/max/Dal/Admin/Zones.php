...
$mzon=mysql_query("select * from ".$table_prefix."zones where zoneid = ".$aZone['zoneid']." and (masterzone =-1 or masterzone =-2 or masterzone =-3 or masterzone =-4) ")or die('error2');
    	if(mysql_num_rows($mzon)>0){
    
		    
       $aRow = mysql_fetch_assoc($mzon);
    ...
    function countZones($agencyId, $campaignId = null, $returnLinked = null, $searchPhrase = null, $includeEmailZones = false) {
        ...
        $aQuery['select'] = "
            ...
            //    $aQuery['where'] .= 'and (masterzone =-1 or masterzone =-2 or masterzone =-3 or masterzone =-4)';
...
/* openxmods - DAC009 */
		$insertZones = $this->oDbh->exec($query);
		
$_page = basename($_SERVER['SCRIPT_NAME']);

	if($_page != 'banner-zone.php'){

		foreach($aZonesIds as $_zoneId) {

			$_query = mysql_query("select * from {$prefix}zones where zoneid = ".$_zoneId." and masterzone = '-2'") or die("Error-1-->".mysql_error());

			if(mysql_num_rows($_query) > 0) {

					$child = $_zoneId;

					$_childZoneIds[1] = ++$child; $_childZoneIds[2] = ++$child; 

					$_childZoneIds[3] = ++$child; $_childZoneIds[4] = ++$child;

			$_bannerQuery = mysql_query("select * from {$prefix}banners where campaignid = ".$campaignId." and masterbanner = '-2'") or die("Error-2-->".mysql_error());
				if(mysql_num_rows($_bannerQuery) > 0) {

					while($_bannerRow = mysql_fetch_assoc($_bannerQuery)) {

						$_getChildBanner = mysql_query("select * from {$prefix}banners where masterbanner = ".$_bannerRow['bannerid']." order by bannerid ASC") or die("Error-3-->".mysql_error());

						if(mysql_num_rows($_getChildBanner) > 0) {

							$v = 1;

							while($_childBannerRow = mysql_fetch_assoc($_getChildBanner)) {

		 mysql_query("INSERT INTO {$prefix}ad_zone_assoc (zone_id,ad_id,priority,link_type,priority_factor) VALUES ('".$_childZoneIds[$v]."','".$_childBannerRow['bannerid']."','0','1','1')") or die("Error-4-->".mysql_error());
...
$_page = basename($_SERVER['SCRIPT_NAME']);
	...
	if($_page != 'banner-zone.php'){
		...
		foreach($aZonesIds as $_zoneId) {
			...
			$_bannerQuery = mysql_query("select * from {$prefix}banners where campaignid = ".$campaignId." and masterbanner = '-2'") or die("Error-2 in-->".mysql_error());
				if(mysql_num_rows($_bannerQuery) > 0) {
...
$_page = basename($_SERVER['SCRIPT_NAME']);
	...
	if($_page != 'banner-zone.php'){
		...
		foreach($aZonesIds as $_zoneId) {
			...
			$_query = mysql_query("select * from {$prefix}zones where zoneid = ".$_zoneId." and masterzone = '-2'") or die("Error-5-->".mysql_error());
			...
			$_bannerQuery = mysql_query("select * from {$prefix}banners where campaignid = ".$campaignId." and masterbanner = '-2'") or die("Error-6-->".mysql_error());
				if(mysql_num_rows($_bannerQuery) > 0) {

					while($_bannerRow = mysql_fetch_assoc($_bannerQuery)) {

						$_getChildBanner = mysql_query("select * from {$prefix}banners where masterbanner = ".$_bannerRow['bannerid']." order by bannerid ASC") or die("Error-7-->".mysql_error());
	 ...
	 mysql_query("delete from {$prefix}ad_zone_assoc where zone_id = ".$_childZoneIds[$v]." and ad_id = ".$_childBannerRow['bannerid']."") or die("Error-8-->".mysql_error());
