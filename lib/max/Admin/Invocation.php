...
$zoneid = $_GET['zoneid'];
 ...
 $adver1=mysql_query("select * from ".$table_prefix."zones where zoneid='".$zoneid."' ")or die('error1'); 
	        $adver=mysql_query("select * from ".$table_prefix."zones where zoneid='".$zoneid."' AND (delivery= '6' or delivery=9)")or die('error2'); 
	        $res =mysql_num_rows($adver);
                $res1=mysql_fetch_array($adver1);


      $adver5=mysql_query("select * from ".$table_prefix."zones where zoneid='".$zoneid."' and masterzone= '-2'")or die('error'); 
	        $res =mysql_num_rows($adver5);
            ...
            foreach($invocationTags as $pluginKey => $invocationTag) {
                        ...
                        if(($res1['delivery']==6 && $invocationTag->getName() =="VAST Ad Tag") || ($res1['delivery']==9 && $invocationTag->getName() =="VAST Ad Tag"))
{
 $aOrderedComponents[$invocationTag->getOrder()] =
			...
			if((mysql_num_rows($adver) > 0) && $invocationTag->getName() == "Rich Media Ad Tag") {
...
else	if((mysql_num_rows($adver5) > 0) && $invocationTag->getName() == "Mobile Tag") {
...
elseif(($invocationTag->getName() != "Inline Video Ad Tag")  && ($invocationTag->getName() != "VAST Ad Tag")  && (mysql_num_rows($adver) == 0) && ($res1['delivery']!=7) && ($res1['delivery']!=6) && ($res1['delivery']!=9) && ($invocationTag->getName() != "Mobile Tag")  && (mysql_num_rows($adver5) == 0)) {
   ...
   //=============================Modified by DAC015===============================//        
		...
		/*DAC020 - Hide the mobile scripts in Direct selection method.*/   
      
           	foreach($invocationTagsNames as $pluginKey => $invocationTagName)
		{				
			if($zone_invocation!='1')
			{			
						if($pluginKey!='invocationTags:oxInvocationTags:mobileadlayer' &&
						 $pluginKey!='invocationTags:oxInvocationTags:mobileadjs' &&
						 $pluginKey!='invocationTags:oxInvocationTags:mobileadview' &&
						 $pluginKey!='invocationTags:oxInvocationTags:mobileadframe' &&
	 					 $pluginKey!='invocationTags:oxInvocationTags:rtbadlayer' &&
						 $pluginKey!='invocationTags:oxInvocationTags:rtbadjs' &&
						 $pluginKey!='invocationTags:oxInvocationTags:housepassbackajs' && 
						 $pluginKey!='invocationTags:oxInvocationTags:housepassbackavw' && 
						 $pluginKey!='invocationTags:oxInvocationTags:rtbadframe' &&
						 $pluginKey!='invocationTags:oxInvocationTags:housepassbackafr' && 
						 $pluginKey!='invocationTags:oxInvocationTags:housepassbackal')
						 {	
								
						$buffer .= "<option value='".$pluginKey."'".($this->codetype == $pluginKey ? ' selected' : '').">".$invocationTagName."</option>";
						 } 
			}
			else if($zone_invocation=='1')
			{ 
						$buffer .= "<option value='".$pluginKey."'".($this->codetype == $pluginKey ? ' selected' : '').">".$invocationTagName."</option>";
			}		
    ...
    public function getTextAreaAndOptions($invocationTag, $extra)
    {
        ...
        // Hide when integrated in zone-advanced.php
if ($this->codetype != 'invocationTags:oxInvocationTags:vastjs'  ) {
        if (!(is_array($extra) && isset($extra['zoneadvanced']) && $extra['zoneadvanced'])) {
            // Header
            // Parameters Section
            $buffer .= "<table border='0' width='100%' cellpadding='0' cellspacing='0'>";
