...
$query_val="select * from 
 ".OX_escapeIdentifier($conf['table']['prefix'].$conf['table']['trackers'])." AS t,
".OX_escapeIdentifier($conf['table']['prefix'].$conf['table']['banners'])." AS b,
".OX_escapeIdentifier($conf['table']['prefix'].$conf['table']['campaigns_trackers'])." AS c,
".OX_escapeIdentifier($conf['table']['prefix'].$conf['table']['campaigns'])." AS cl
where cl.campaignid=c.campaignid  and c.trackerid=t.trackerid  and  c.cookieless_tracker='1' and c.campaignid=b.campaignid and bannerid='$adId'";
...
if($results['cookieless_tracker']=='1')
{
		...
		$trackerid=$results['trackerid'];

		list($usec, $sec) = explode(' ', microtime());

		$time = (float)$usec + (float)$sec;

		$date_time = date('Y-m-d H:i:s',mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y')));


		$viewerId = MAX_cookieGetCookielessViewerID();
		
		$create_unique_user=$date_time.$viewerId.$adId.$zone_id.$trackerid;

		$transaction_id = substr(md5($create_unique_user), 0, 32);

		$converto_str=implode(',',$aVariables);

		$dest=str_replace('{transaction_id}',$transaction_id,$dest);
