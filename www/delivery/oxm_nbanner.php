...
$qryCount=OA_Dal_Delivery_query("SELECT ipranges as cc FROM rv_campaign_iprange WHERE campaignid = '".$aAd['placement_id']."' ");
$rowCount=mysql_fetch_row($qryCount);
if(preg_replace('/\s+/', '', $rowCount[0])!=""){
	$qry=OA_Dal_Delivery_query("SELECT ipranges FROM rv_campaign_iprange WHERE campaignid = '".$aAd['placement_id']."' ");
...
//pageRedirection($aAd['placement_id'],'no');
...
$valuequery = OA_Dal_Delivery_query("select ad_id from ".$table_prefix."ad_zone_assoc where zone_id=".$_GET['zoneid']."  $condition ");
		$i = 0;
		while($row = OA_Dal_Delivery_fetchAssoc($valuequery))
		{

			$what = 'bannerid:'.$row['ad_id'].'';

			$outputs[] = MAX_adSelect($what, $campaignid, $target, $source, $withtext, $charset, $context, true, $ct0, $GLOBALS['loc'], $GLOBALS['referer']);
		}
	
	
$output = "<script> var a = new Array();";

$i = 0;

$refresh=$_GET['refresh'];
if (isset($refresh) && is_numeric($refresh) && $refresh > 0) {
$refresh = $_GET['refresh']*1000;
}
else
{
$refresh = '5000'; 
...
//	$temp="";	
//	str_replace('"',"'",$item['html'],$temp);
	$lg = "<img style='visibility:hidden;' src='".$item['logUrl']."'>";
	$output .= "a[".$i."] = \"".$lg.$item['html']."\";";
	$i++;
...
#test a img{ display:block;}


</style>
<script>

$(a[0]+' img').each(function(i){

				var img1 = '';
				var img2 = '';
				if(img1 = this.src){
					
					this.src = img1+'?'+new Date().getTime();
					//console.log(this.src);
				}
				if(img2 = $($(this).find('img')[0]).attr('src')){
					$($(this).find('img')[0]).attr('src',img2+'?'+new Date().getTime());
					//console.log($($(this).find('img')[0]).attr('src'));
				}	
			}) 			
...
setInterval(function(){	 
	      	...
	      	$(a[interval]+' img').each(function(i){
				var img1 = '';
				var img2 = '';
				if(img1 = this.src){
		...
		$('#test').html('');			
		$('#test').html(a[interval]);		
						
		if(a.length - 1 > interval){
...
echo MAX_javascriptToHTML($output, 'OX_'.substr(md5(uniqid('', 1)), 0, 8));
