<?php
define('LOG_FILE_204','../../logs/204.7.'.date('Y-m-d').'.log');
...
function djax_getAd($adid,$id)
{
		...
		else
		{
			...
			error_log(date('Y-m-d H:i:s').'|'.$bid_id.'|'.$aAd['ad_id'].'|1|'.$bid_floor.'|price is below requested price'.PHP_EOL,3,LOG_FILE_204);
		}
		//$aRows[$aAd['ad_id']] = $aAd;
		$dbh = null;	
