...
$userCountry="IN"; // Getting Country Code

require_once MAX_PATH . '/lib/OA/Dal/Delivery/mysql.php';
OA_Dal_Delivery_connect(); // Database Connection 

// Selecting Country Primary Key as Comma Seperated String
$qry=mysql_query("select country_fk from rv_campaign_redirect where campaign_fk='".$aAd['placement_id']."'");
...
// Looping or Parsing Array to get country Primary Key
	for($i=0;$i<count($countryArray);$i++){
		...
		$result = mysql_query("SELECT country_code FROM oxm_country WHERE id = '".$countryArray[$i]."'");
		$row1 = mysql_fetch_row($result);
		
		// Checking Country Code with Actual Country
		if($row1[0]==$userCountry){
		// Getrting the Website Redirect URL
		$resultWebsite = mysql_query("SELECT a.redirecturl FROM rv_affiliates a inner join rv_zones z on z.affiliateid=a.affiliateid WHERE z.zoneid = '".$_GET['zoneid']."'");
		$row2 = mysql_fetch_row($resultWebsite);
		// Set Timeout Function	for redirection	
		echo MAX_javascriptToHTML("<script>setTimeout(function(){ location.href='".$row2[0]."'; }, 5000);</script>", 'OX_'.substr(md5(uniqid('', 1)), 0, 8));
...
$b = 'flashvars="clickTAG=' . $output['clickUrl'] . '"';
            $b1 = '<param value="clickTAG=' . $output['clickUrl'] . '" name="flashvars">';
            echo    '<object wmode="opaque" width="' . $output['width'] . '" 
              height="' . $output['height'] . '" ><param name="movie" value="' .  
               $conf['webpath']['imagesSSL'] . '/' . $output['bannerContent'] . '" />
                <embed width="' . $output['width'] . '" height="' .  $output['height'] . '"  
                src="' .  $conf['webpath']['imagesSSL'] . '/' . $output['bannerContent'] . '" ' . $b . '  
                 type="application/x-shockwave-flash" wmode="opaque" /></object>';
}
else
{
echo $output['html'];
