...
{
MAX_header('Content-Type: text/plain');
...
if (empty($target)) {
$target = '_blank';  }
$outputbuffer = $g_prepend . '<div><a href=\'' . $row['default_banner_destination_url'] . '\' target=\'' .
$target . '\'><img src=\'' . $row['default_banner_image_url'] .
'\' border=\'0\' alt=\'\'></a></div>' . $g_append;
...
if (empty($target)) {
$target = '_blank';  }
$outputbuffer = "<div>{$g_prepend}<img src='{$conf['defaultBanner']['imageUrl']}' border='0' alt=''>{$g_append}</div>";
...
echo "console.log('Hi');";
//echo "document.write('<script>http://192.168.1.200/development/alam_dev/ads/www/delivery/banners.js?zoneid=326</script>');";
...
// Looping or Parsing Array to get OS Primary Key
	for($i=0;$i<count($osArray);$i++){
		...
		// Checking OS Name with Actual OS
		if($row1[0]==$osType){
