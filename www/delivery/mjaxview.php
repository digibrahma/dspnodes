...
//=======================================Modified by DAC021=========================================//
	...
	$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
		...
		$query = OA_Dal_Delivery_query("select * from oxm_mobilezones where masterzoneid =".$zoneid ) or die("culnot select1:".mysql_error());
...
{
//print_r($row['compiledlimitation']);die("hai");
if (!empty($row['compiledlimitation'])) {
if (!isset($GLOBALS['_MAX']['FILES']['aIncludedPlugins'])) {
$GLOBALS['_MAX']['FILES']['aIncludedPlugins'] = array();
}
...
$details = mobileParameters('smart');
...
if($limitation=='Smartphone,Non Smartphone')
{
	return true;
}
	
elseif($limitation=='Non Smartphone' && empty($details))
{
	return true;
}
elseif($limitation=='Smartphone' && $details==1)
{
	return true;
}
else
...
function teleco($data,$op)
{

	//$_SERVER['REMOTE_ADDR']="223.189.12.161";
	
	$ipnum= ip2long($_SERVER['REMOTE_ADDR']);
										
	$tt=OA_Dal_Delivery_fetchAssoc(OA_Dal_Delivery_query("SELECT carriername as teleco FROM `djax_carrier_detail` WHERE '".$ipnum."' BETWEEN start_ip AND end_ip"));	
...
{ 
	...
	$details = mobileParameters('model');
		...
		if(!empty($details)) {
		 	...
		 	return MAX_limitationsMatchStringValue($details, $limitation, $op);
...
//print_r($all_values);die("hai");
	/*SMARTPHONE*/
	$v=objectToArray($all_values);

	$smartphone=$v['device']['virtualcapability']['8'];
	$ss=$smartphone['@attributes']['value'];
print_r($v['device']);
die();

	$sm=$v['device']['virtualcapability']['18'];//form_factor
	if($type == 'handset') {
		$hanset_brand = $all[24]['value']; //device Brand
		return $hanset_brand;
	} elseif($type == 'os') {
		$hanset_os = $all[5]['value']; //print_r($hanset_os);die("aslk");  //device OS
		return $hanset_os;
	} elseif($type == 'model') {
		$hanset_model = $all[17]['value']; //print_r($hanset_model);die("ahgh"); //device Model
		return $hanset_model;
	}
...
//===========================================================Modified by DAC021=====================================================================================//
	...
	$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
		...
		$query = OA_Dal_Delivery_query("select * from oxm_mobilezones where masterzoneid =".$_GET['zoneid'] ) or die("culnot select:2".mysql_error());
