...
if (!empty($bannerid))
{
	...
	/* openxmods - DAC009 */
	$_query = mysql_query("select * from ".$table_prefix."banners where masterbanner = ".$bannerid."") or die(mysql_error());

	if(mysql_num_rows($_query) > 0) {

		while($_row = mysql_fetch_assoc($_query)) {

		    $_doBanners = OA_Dal::factoryDO('banners'); 
		    $_doBanners->get($_row['bannerid']);

		    $_doBanners->status = $value;
		    $_doBanners->update();
