...
while ($doBanners->fetch() && $row = $doBanners->toArray()) {
	...
	if((($row['masterbanner'] == -1) || ($row['masterbanner'] == -2)))
	{
		$query = mysql_query("select * from rv_banners where masterbanner =".$row['bannerid']) or die(mysql_error());
		if(mysql_num_rows($query) > 0)
		{

			$banners[$row['bannerid']]['mobitype'] = 'mobile';
		} else {
			$banners[$row['bannerid']]['mobitype'] = 'web';
		}
	}
	
	if($row['masterbanner']==-4)
	{
		$banners[$row['bannerid']]['mobitype'] = 'html';
