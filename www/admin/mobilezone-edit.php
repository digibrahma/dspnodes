...
+---------------------------------------------------------------------------+
$Id: zone-edit.php 49423 2010-02-26 20:19:26Z chris.nutting $
*/
...
$query = mysql_query("select * from oxm_mobilezonesize where user_id=1 order by id ASC limit 0,1") or die(mysql_error());
...
//======================================================Modified By DAC021============================================//
    $zone['width']           = $row['width'];
    $zone['height']          = $row['height'];
...
//======================================================Modified By DAC021============================================//
         ...
         $header = $form->createElement('header', 'header_sql', "Mobile Parent Zone Size : ");
    ...
    $aCustomSize['width'] = $form->createElement('text', 'width', $GLOBALS['strWidth'].':', 'disabled = "disabled"',
    ...
    $aCustomSize['height'] = $form->createElement('text', 'height', $GLOBALS['strHeight'].':', 'disabled = "disabled"',
        array('onkeydown' => 'phpAds_formEditSize();',
            'onChange' => 'oa_sizeChangeUpdateMessage("warning_change_zone_size");'));
    ...
    if (empty($errors)) {
        ...
        if (!empty($aFields['zoneid']))
        {
            ...
            $translated_message = $translation->translate ( $GLOBALS['strZoneHasBeenUpdated'],
                array(
                MAX::constructURL(MAX_URL_ADMIN, "mobilezone-edit.php?affiliateid=".$aFields['affiliateid']."&zoneid=".$aFields['zoneid']),
                htmlspecialchars($aFields['zonename'])
                ));
            OA_Admin_UI::queueMessage($translated_message, 'local', 'confirm', 0);

            OX_Admin_Redirect::redirect("mobilezone-edit.php?affiliateid=".$aFields['affiliateid']."&zoneid=".$aFields['zoneid']);
        }
        // Add
        else
        {
...
$row = mysql_fetch_assoc($query);
            ...
            $doZones->width = $row['width'];
            $doZones->height = $row['height'];
//======================================================Modified By DAC021============================================//
            $doZones->delivery = $aFields['delivery'];
            $doZones->oac_category_id  = $aFields['oac_category_id'];
            ...
            $translation = new OX_Translation ();
//======================================================Modified By DAC021============================================//
$desc = "Parent Zone ".$aFields['zonename']." has been added with Four Mobile Zone";
...
$query = mysql_query("select * from oxm_mobilezonesize where user_id=1 order by id ASC") or die(mysql_error());
$i=1;

	if(empty($aFields['zoneid'])) :

		$aFields['zoneid'] = mysql_insert_id();
	...
	while($row = mysql_fetch_assoc($query))
	{
		    ...
		    $doZones->zonename = $aFields['zonename']." - [ Mobile Zone".$i." ]";
		    $doZones->width = $row['width'];
		    $doZones->height = $row['height'];
		    $doZones->masterzone = $aFields['zoneid'];

	 	   	$newZoneId = $doZones->insert();

			if(empty($newZoneId)) :

				$newZoneId = mysql_insert_id();

			endif;

			mysql_query("update ".$table_prefix."zones set masterzone = ".$aFields['zoneid']." where zoneid =".$newZoneId) or die(mysql_query());
...
$query = mysql_query("select * from ".$table_prefix."zones where masterzone=".$aFields['zoneid']." order by zoneid ASC") or die(mysql_error());

$j =1;
	while($row = mysql_fetch_assoc($query))
	{

			$childZone[$j] = $row['zoneid'];

$j++;
			
	}
//======================================================Modified By DAC016============================================//
mysql_query("update rv_zones set masterzone = -2 where zoneid =".$aFields['zoneid']) or die(mysql_query());


	mysql_query("insert into oxm_mobilezones(masterzoneid,mz1,mz2,mz3,mz4) values(".$aFields['zoneid'].",".$childZone[1].", ".$childZone[2].",".$childZone[3].",".$childZone[4].") ") or die(mysql_error());
