...
if (!empty($campaignid)) {
		   if (isset($action) && $action == 'set' && $_GET['status']!=1) {
   	...
   	$count = array_sum($_POST['cookieless']);

        if($count>1)
	{
    	$translation = new OX_Translation();
        $translated_message = $translation->translate ("Only one Tracker can act as cookie-less tracker for each campaign", array(
            MAX::constructURL(MAX_URL_ADMIN, "campaign-edit.php?clientid=".$clientid."&campaignid=".$campaignid."&status=1"),
            htmlspecialchars($doCampaigns->campaignname)
        ));
        OA_Admin_UI::queueMessage($translated_message, 'local', 'error', 0);

        OX_Admin_Redirect::redirect("campaign-trackers.php?clientid=".$clientid."&campaignid=".$campaignid."&status=1");
        }
            ...
            for ($i=0; $i<sizeof($trackerids); $i++) {
                ...
                for ($k = 0; $k < $fieldsSize; $k++) {
                    ...
                    $doCampaigns_trackers->$field = $values[$k];
                        ...
                        $track[]=$doCampaigns_trackers->insert();


            }

		$cookie=$_POST['cookieless'];

		foreach($cookie as $key=>$value)
		{
			if(!empty($track[$key]))
			{
			$update=mysql_query("UPDATE   rv_campaigns_trackers SET cookieless_tracker='$value' WHERE campaign_trackerid ='".$track[$key]."'");
			}
            }
        }
...
echo "\t\t\t\t\t</td>\n";
 ...
 echo "\t\t\t\t\t<td width='100'>\n";
echo "\t\t\t\t\t\t<b>Global - tracker status</b>\n";
echo "\t\t\t\t\t</td>\n";
echo "\t\t\t\t</tr>\n";
...
if ($doTrackers->getRowCount() == 0) {
    ...
    foreach ($trackers as $tracker) {
 	...
 	echo "\t\t\t\t\t<td height='25'>";
        ...
        echo "<select name='cookieless[]' id='cookielesstrk".$tracker['trackerid']."' tabindex='".($tabindex++)."'>\n";



	$campaign_trackers=mysql_query("SELECT * FROM    rv_campaigns_trackers WHERE trackerid =".$tracker['trackerid']." and campaignid=".$campaignid." ")or die("er1");

	$fetch_campaign_trackers=mysql_fetch_array($campaign_trackers);


$cookieless = $fetch_campaign_trackers['cookieless_tracker'];



 
echo "<option value='0' ". ($cookieless == 0 ? 'selected' : '')." >Default&nbsp;</option>\n";
echo "<option value='1' ". ($cookieless == 1 ? 'selected' : '')." >Cookieless&nbsp;</option>\n";
...
echo "</select>\n";
        ...
        echo "\t\t\t\t\t<td colspan='4'><img src='" . OX::assetPath() . "/images/break-l.gif' height='1' width='100%'></td>\n";
        echo "\t\t\t\t</tr>\n";
            ...
            for (var i=0; i<document.availabletrackers.elements.length; i++)
            {
                ...
                if (element.id.substring(0,3) == 'trk')
                {
                    ...
                    var logelement = document.getElementById('status' + element.id);
					 	    var logelement1 = document.getElementById('cookieless' + element.id);
		    if (logelement1) logelement1.disabled = !element.checked;
