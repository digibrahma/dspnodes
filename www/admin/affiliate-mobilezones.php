...
$oTpl = new OA_Admin_Template('affiliate-mobilezone-index.html');
...
while ($doZones->fetch() && $row_zones = $doZones->toArray()) {
 if(!empty($zoneid)) {
			...
			if(!empty($row_zones['masterzone']) &&  (($row_zones['masterzone'] != -1) || ($row_zones['masterzone'] != -2)) &&  ($row_zones['masterzone']== $zoneid))
			  {		
			...
			if(!empty($row_zones['masterzone']) &&  (($row_zones['masterzone'] != -1) || ($row_zones['masterzone'] != -2)))
			{
				$aZones[$row_zones['zoneid']] = $row_zones;
					$aZones[$row_zones['zoneid']]['lowPriorityWarning'] = false;
