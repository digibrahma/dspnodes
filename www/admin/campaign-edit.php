...
if ($campaignid != "") {
    ...
    $campaign['daily-budget'] = OA_Admin_NumberFormat::formatNumber ( $data ['daily_budget'], 4 );
    $campaign['revenue_type'] = $data ['revenue_type'];
    $campaign['dsp_category'] = isset($data ['dsp_category'])?explode(',',$data ['dsp_category']):'';
	...
	$doRedirect = OA_Dal::factoryDO("campaigns");
	$doRedirect->whereAdd("campaignid=".$campaignid);
	$doRedirect->find();
	 while ($doRedirect->fetch() > 0) {
            // Store the user info for later
            $Camp_info = $doRedirect->toArray();
            
        }

	$campaign['redirect'] = $Camp_info['redirect'];
...
function buildCampaignForm($campaign, &$oComponent = null)
{
    ...
    $form->addElement ( 'submit', 'submit', $GLOBALS ['strSaveChanges'] ,array('onclick'=>'return getClick()'));
...
function buildPricingFormSection(&$form, $campaign, $newCampaign, $remnantEcpmEnabled, $contractEcpmEnabled)
{
   ...
   //dsp category
   	...
   	$query = mysql_query("SELECT value,category_name FROM `djax_site_categories`");
	while($category = mysql_fetch_array($query))
	{
	$dspcategory[$category['value']]=$category['category_name']."&nbsp;&nbsp;-&nbsp;&nbsp;".$category['value'];
	}	

   	//array_unshift($dspcategory, "Select category");

   	
   	
    //$form->addElement ('header', 'Dsp category', 'Dsp category');
    
	if($campaign['campaignid'])
	{
		
		$dspcat = mysql_fetch_array(mysql_query("SELECT category_name,value FROM `rv_campaigns` as c join djax_site_categories as cat on c.dsp_category=cat.value where campaignid='".$campaign['campaignid']."'"));
			
		$dspcategory1[$dspcat['value']]=$category['category_name']."&nbsp;&nbsp;-&nbsp;&nbsp;".$category['value'];
		
		$cc=array_merge($dspcategory1,$dspcategory);
	...
	*/
		...
		//$form->addElement ( 'select', 'dspcategory[]', 'dspcategory', $cc,array ('multiple' => 'multiple','id'=>'dspcategory'));
		//$form->addElement('multiselect', 'somename[]');
		
	}
	else
	{
		//$form->addElement ( 'select', 'dspcategory[]', 'dspcategory', $dspcategory,array ('multiple' => 'multiple'));
	}
   ...
   //dsp category
    ...
    //rate price - common
    $ratePriceG ['field'] = $form->createElement ( 'text', 'revenue', null,'required');
   ...
   //Daily budget - dac041
        ...
        $form->addElement ( 'text', 'daily-budget','Daily Budget',array ('id' => 'daily-budget', 'class' => 'small' ));
...
OA_Dal_Delivery_connect(); // Database Object Creation
	
	
	$form->addElement('header', 'header_misc', "Redirect Campaign");
		
	
   	$redirect = ($campaign['redirect']=='1')?'checked':"";

 $form->addElement('static', 'tar',"<input type='checkbox' id='redirect' name='redirect' value='1'". $redirect ."><b style='color:red;'>Redirect Campaign</b>",'');
	
	
	$form->addElement('header', 'header_dsp', "DSP Portal");
	...
	$dbHolder	=	"";
    $rtb 		=	($campaign['rtb']=='1')?'checked':"";
    $q			=	mysql_query("select * from rv_dj_dsp where status='1'");
    $query 		= 	mysql_query("SELECT value,category_name FROM `djax_site_categories`");
	while($category = mysql_fetch_array($query))
	{
		$dspcategory[$category['value']]=$category['category_name']."&nbsp;&nbsp;-&nbsp;&nbsp;".$category['value'];
	...
	{
		$camps			=	mysql_query("select dsp_portals from rv_campaigns where campaignid='".$_GET['campaignid']."'");
		$camps_portal	=	mysql_fetch_row($camps);
		$temp			=	explode (",", $camps_portal[0]);
		while($row=mysql_fetch_array($q))
		{
			...
			//$form->addElement('header', 'header_dsp'.$k, "DSP Portal".$k);
			if($control==1)
			{
				// Get Selected categories value from table for each Ad exchanges.Added on 25-09-2015 By sakthivel.P 
				...
				foreach($dspcategory as $key=>$value):  
					...
					{
						$dbHolder.="<option value='".$key."' selected='selected'>".$value."</option>";
					}
					else
					{
						$dbHolder.="<option value='".$key."'>".$value."</option>";
					}
				endforeach;			
				$dbHolder.="</select></td></tr>";
			}
			else
			{
 ...
 $form->addElement('static', 'tar','<div style="width:100%; "><table><tr   height="30"><th colspan="2"><b>Select DSP Portal\'s</b></th></tr>
 '.$dbHolder.'
 </table></div>
','');
...
function processCampaignForm($form, &$oComponent = null)
{
    ...
    if (empty($errors)) {
        ...
        if (!empty($aFields['campaignid']) && $aFields['campaignid'] != "null") {
			$dspVal="";
			$dspVal=implode (",", $_POST['dspPortals']);
			
            $doCampaigns->campaignid = $aFields['campaignid'];
            $doCampaigns->setEcpmEnabled();
            $doCampaigns->update();
            if(isset($_POST['dspPortals']) && $_POST['dspPortals']!='')
            {
					// Delete the previous entry of corresponding Campaign from Portal category table.Added on 25-09-2015 By sakthivel.P
					
				if(mysql_query("delete from rv_dsp_portal_categories where campaign_id='".$aFields['campaignid']."'"))
				{
					foreach($_POST['dspPortals'] as $values):
					
					mysql_query("insert into rv_dsp_portal_categories(campaign_id,portal_id,categories) values('".$aFields['campaignid']."','".$values."','".$_POST[$values]."')");
					endforeach;
				}
				
			}
			            $dsp	=	'';
			if(isset($_POST['dspcategory']) && $_POST['dspcategory']!='')
			{
				$dsp	=	implode(',',$_POST['dspcategory']);
			}
          
            mysql_query("update rv_campaigns set redirect='".$_POST['redirect']."',daily_budget='".$_POST['daily-budget']."',daily_budget_remaining='".$_POST['daily-budget']."', dsp_category='".$dsp."' where campaignid='".$aFields['campaignid']."'");
                       
           /*
            mysql_query("update rv_campaigns set redirect='".$_POST['redirect']."' where campaignid='".$aFields['campaignid']."'");
            mysql_query("update rv_campaigns set dsp_portals='".$dspVal."' where campaignid='".$aFields['campaignid']."'");
            mysql_query("update rv_campaigns set dsp_category='".$_POST['dspcategory']."' where campaignid='".$aFields['campaignid']."'"); */
            ...
            mysql_query("update rv_campaigns set redirect='".$_POST['redirect']."',daily_budget='".$_POST['daily-budget']."',daily_budget_remaining='".$_POST['daily-budget']."', dsp_category='".isset($_POST['dspcategory'])?implode(',',$_POST['dspcategory']):''."'  where campaignid='".$aFields['campaignid']."'");
            // insert Portal Category.Added on 25-09-2015 By sakthivel.p
            foreach($_POST['dspPortals'] as $values):
...
function displayPage($campaign, $campaignForm, $statusForm, $campaignErrors = null)
{
    ...
    $oTpl->assign ( 'dsp_category', implode("','",$campaign['dsp_category']));
    //$campaign['dsp_category'] = isset($data ['dsp_category'])?explode(',',$data ['dsp_category']):'';
