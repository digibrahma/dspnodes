...
$oTpl = new OA_Admin_Template('campaign-mobilebanner-index.html');


$doBanners = OA_Dal::factoryDO('banners');
$doBanners->campaignid = $campaignid;
$doBanners->masterbanner = $bannerid;
...
while ($doBanners->fetch() && $row = $doBanners->toArray()) {
   ...
   if(!empty($bannerid)) {
	if(!empty($row['masterbanner']) &&  (($row['masterbanner'] != -1) || ($row['masterbanner'] != -2)) && ($row['masterbanner'] == $bannerid))
	{
	...
	if(!empty($row['masterbanner']) && (($row['masterbanner'] != -1) || ($row['masterbanner'] != -2)))
	{
