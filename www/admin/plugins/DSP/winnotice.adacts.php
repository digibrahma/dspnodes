...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
	...
	//	error_log(print_r($_REQUEST, TRUE), 3, 'notice.log');
		...
		//$fetchdata=mysql_fetch_assoc(mysql_query("select id from {$table_prefix}dj_adacts_bid_request where bid_request_id='$bidid'"));
		
		/********** Insert Win notice into AFF_SMAATO_WIN_NOTICE **********/
		$sql = "INSERT INTO  
						`{$table_prefix}dj_adacts_win_notice` (
							`datetime`,
							`auctionID`,
							 request_id,
							`bidid` ,
			...
			//mysql_query("update {$table_prefix}dj_adacts_response set win_notice='1' where id='".$bidid."'");	
		...
		// if(mysql_query("update {$table_prefix}dj_adacts_response set win_notice='1' where id='".$bidid."'"))
		 {
			 //error_log("update {$table_prefix}dj_adacts_response set win_notice='1' where id='".$bidid."'",3,"error.log");
