...
function buildBannerForm($type, $aBanner, &$oComponent=null, $formDisabled=false)
{
    ...
    if ($type == 'sql' || $type == 'web') {
		...
		$form->addElement('header', 'header_b_display', 'Redirect'); 
		$query = mysql_query('SELECT `redirect`,`repopup` FROM `rv_banners` WHERE `bannerid` = '.$aBanner['bannerid']);

$fetch = mysql_fetch_array($query);

$aBanner['redirect'] = $fetch['redirect'];
$aBanner['repopup'] = $fetch['repopup'];


if(!empty($aBanner['redirect'])){

    $form->addElement('checkbox', 'redirect', '','Redirect',array('id' => 'redirect','onchange'=>'direction(this)','checked'=>'checked'));
    $form->addElement('radio', 'repopup', 'Popup', '','popup');
    $form->addElement('radio', 'repopup', 'URL', '', 'url');

}else{

    $form->addElement('checkbox', 'redirect', '','Redirect',array('id' => 'redirect','onchange'=>'direction(this)'));
    $form->addElement('radio', 'repopup', 'Popup', '','popup'); 
    $form->addElement('radio', 'repopup', 'URL', '', 'url');
   ...
   $res=mysql_query("select * from rv_banners where masterbanner='-2' and bannerid=".$aBanner['bannerid'])or die("Error--->1".mysql_error());
...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
    ...
    if ($insert) {
        $bannerid = $doBanners->insert();
         ...
         if($_POST['redirect'] == 1 && $_POST['repopup'] != ''){
          mysql_query('UPDATE `rv_banners` SET `redirect` = "'.$_POST['redirect'].'", `repopup` = "'.$_POST['repopup'].'"  WHERE `bannerid` = '.$bannerid );
        }else{
            mysql_query('UPDATE `rv_banners` SET `redirect` = "0", `repopup` = ""  WHERE `bannerid` = '.$bannerid );
        } 
        
//======================================================Modified By DAC021============================================//
mysql_query("update ".$table_prefix."banners set masterbanner = -1 where bannerid =".$bannerid) or die("Error---> 3".mysql_error());
//======================================================Modified By DAC021============================================//
       ...
       $res1=mysql_query("select * from ".$table_prefix."banners where masterbanner='-2' and bannerid=".$bannerid)or die("Error--->4".mysql_error());
...
var input = form.elements.repopup;
...
<?php
if($aBanner['redirect'] == 0){
?>
input[0].closest('tr').style = 'display:none';
input[1].closest('tr').style = 'display:none';
<?php
}else{
?>
input[0].closest('tr').style = 'display:table-row';
input[1].closest('tr').style = 'display:table-row';
<?php
}
?>
